...
- baseurl (updated 2026-07-07 after the org transfer): the repo is now the org PROJECT repo `BattModels/ziqiwang`, served at a custom-domain subpath `https://eeg.engin.umich.edu/ziqiwang/`. So `_config.yml` has `baseurl: /ziqiwang` and `url: https://eeg.engin.umich.edu`. Do NOT set baseurl empty (that was correct only for the old user-site repo). Local `jekyll serve` also serves under `/ziqiwang/` (open `localhost:<port>/ziqiwang/`). All internal links use `relative_url`/`figure.liquid`, so they follow baseurl automatically; if assets 404, check baseurl first. No CNAME file in this repo: the custom domain is inherited from the org site.
...
5. **Project: MLIP**: SKIPPED indefinitely (Ziqi, 2026-07-09: "skip the MLIP page forever until I tell you so"). Do not build it or add a tile until Ziqi lifts this. MLIP still appears as a research-experience entry in the inline resume, which is fine.

Structural change 2026-07-06 (consolidated onto About; supersedes the separate-pages plan above), updated 2026-07-09:
- **Single-page site.** Everything lives on the About page (`_pages/about.md`): intro + headline results, project tiles (looped from `_projects` via the `projects.liquid` card include, ordered by `importance`), and an inline Resume section.
- **Nav is a scroll index (2026-07-09).** The navbar shows About / Projects / Resume. Projects and Resume are NOT pages: they are anchor links (`/#projects`, `/#resume`) to sections on the About page. Clicking scroll-jumps there; a scrollspy script in `about.md` highlights the active tab on scroll. This required a local override of `_includes/header.liquid` (registered in `.al-folio-overrides.yml`; two `<li data-scrollspy>` items added after About). The scrollspy JS + all resume/section CSS live in a `<style>`/`<script>` block at the bottom of `about.md` (scoped to the home page). If the al_folio_core gem is upgraded, re-check the header override.
- **Resume section** is styled as a two-column academic CV (date/label left, content right; uppercase section rules), inspired by otiliastr.github.io/cv but keeping al-folio's theme colors via `--global-*` CSS variables. Content is transcribed from the PDF; re-transcribe if the PDF changes.
...
- **CV page removed** (`_pages/cv.md` deleted; no more `al_folio_cv`/RenderCV). The resume is the PDF at `assets/html/Ziqi_Wang_Resume_2026.pdf`, opened via a button on the About Resume section (target=_blank, browser renders it), plus the inline text resume.
- The old projects index (`_pages/projects.md`) still exists but is `nav: false` and is now redundant (tiles are on About).
...
- Local preview, in priority order:
  1. User-space Ruby if available (`module avail ruby`, then `bundle config set --local path vendor/bundle`, `bundle install`, `module load ruby/3.3.7 && bundle exec jekyll serve --port <high uncommon port> --force_polling`). ALWAYS pass `--force_polling`: the repo is on NFS and Jekyll's inotify watcher does not receive change events there, so without it the server serves the initial build forever and edits never show on refresh (symptom: `_site/.../index.html` mtime stays older than the edited source). Drop `--livereload` (its blocking script at the top of `<head>` hangs the page if its port is not also forwarded); refresh the browser manually after each save. Open `localhost:<port>/ziqiwang/` (baseurl is `/ziqiwang`, so the site does NOT serve at the root path). If the page hangs at 0 bytes while `curl` on the node is instant, it is the login-node mismatch (VS Code attached to a different Lighthouse node than the server); diagnose with `hostname` and use a laptop `ssh -N -L <port>:127.0.0.1:<port> ziqiw@lh-login1.arc-ts.umich.edu` tunnel.
